...
export default class ListCommand extends Command {
	 ...
	 * @param {'numbered'|'bulleted'|'star'|'dash'|'todo'} type List type that will be handled by this command.
	 */
	constructor( editor, type ) {
		 ...
		 *
		 * @readonly
		 * @member {'numbered'|'bulleted'|'star'|'dash'|'todo'}
		 */
		this.type = type;
		...
		model.change( writer => {
			...
			for ( const element of blocks.reverse() ) {
				if ( turnOff && element.name == 'listItem' ) {
					...
					console.log({ listType: this.type, listIndent: 0 }, element)
					...
					console.log('listType', this.type, element)
