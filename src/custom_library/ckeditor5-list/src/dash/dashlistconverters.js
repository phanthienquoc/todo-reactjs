...
/**
 * @module list/dashlistconverters
 */
...
export function modelViewInsertion( model, onCheckboxChecked ) {
	return ( evt, data, conversionApi ) => {
		...
		if ( data.item.getAttribute( 'listType' ) != 'dash' ) {
			return;
		}
		...
		consumable.consume( modelItem, 'attribute:dashListChecked' );

		const viewWriter = conversionApi.writer;
		const viewItem = generateLiInUl( modelItem, conversionApi );

		const isChecked = !!modelItem.getAttribute( 'dashListChecked' );
		const checkmarkElement = createCheckmarkElement( modelItem, viewWriter, isChecked, onCheckboxChecked );

		viewWriter.addClass( 'dash-list', viewItem.parent );
...
export function dataModelViewInsertion( model ) {
	return ( evt, data, conversionApi ) => {
		...
		if ( data.item.getAttribute( 'dashListChecked' ) ) {
			viewWriter.setAttribute( 'checked', 'checked', checkbox );
			viewWriter.addClass( 'dash-list__label', label );
		}
...
export function dataModelViewTextInsertion( evt, data, conversionApi ) {
	...
	if ( parent.name != 'listItem' || parent.getAttribute( 'listType' ) != 'dash' ) {
		return;
	}
	...
	const span = viewWriter.createAttributeElement( 'span', { class: 'dash-list__label__description' } );
 ...
 * When a view checkbox element is marked as checked, an additional `dashListChecked="true"` attribute is added to the model item.
 *
 * It is used by {@link module:engine/controller/datacontroller~DataController}.
 *
...
export function dataViewModelCheckmarkInsertion( evt, data, conversionApi ) {
	...
	if ( viewItem.getAttribute( 'type' ) != 'checkboxdash' || modelItem.name != 'listItem' || !modelCursor.isAtStart ) {
		return;
	}
	...
	writer.setAttribute( 'listType', 'dash', modelItem );

	if ( data.viewItem.hasAttribute( 'checked' ) ) {
		writer.setAttribute( 'dashListChecked', true, modelItem );
	}
 ...
 *
 * This change means that the `<li>` element parent changes to `<ul class="dash-list">` and a
...
export function modelViewChangeType( onCheckedChange, view ) {
	return ( evt, data, conversionApi ) => {
		...
		if ( data.attributeNewValue == 'dash' ) {
			const isChecked = !!data.item.getAttribute( 'dashListChecked' );
			const checkmarkElement = createCheckmarkElement( data.item, viewWriter, isChecked, onCheckedChange );

			viewWriter.addClass( 'dash-list', viewItem.parent );
			viewWriter.insert( viewWriter.createPositionAt( viewItem, 0 ), checkmarkElement );
		} else if ( data.attributeOldValue == 'dash' ) {
			viewWriter.removeClass( 'dash-list', viewItem.parent );
...
/**
 * A model-to-view converter for the `dashListChecked` attribute change on the `listItem` model element.
 *
...
export function modelViewChangeChecked( onCheckedChange ) {
	return ( evt, data, conversionApi ) => {
		// Do not convert `dashListChecked` attribute when to-do list item has changed to other list item.
		// This attribute will be removed by the model post fixer.
		if ( data.item.getAttribute( 'listType' ) != 'dash' ) {
			return;
		}

		if ( !conversionApi.consumable.consume( data.item, 'attribute:dashListChecked' ) ) {
			return;
		}
 ...
 *
 * This helper ensures that position inside dash-list in the view is mapped after the checkbox.
 *
...
export function mapModelToViewZeroOffsetPosition( view, mapper ) {
	return ( evt, data ) => {
		...
		// Handle only position at the beginning of a dash list item.
		if ( !parent.is( 'listItem' ) || parent.getAttribute( 'listType' ) != 'dash' || modelPosition.offset !== 0 ) {
			return;
		}
