...
import DashListCheckCommand from './dashlistcheckcommand';
 ...
 */
export default class DashListEditing extends Plugin {
	/**
	 * @inheritDoc
	 */
	static get pluginName() {
		return 'DashListEditing';
	}
	...
	init() {
		...
		model.schema.extend( 'listItem', {
			allowAttributes: [ 'dashListChecked' ]
		} );

		// Disallow dashListChecked attribute on other nodes than listItem with to-do listType.
		model.schema.addAttributeCheck( ( context, attributeName ) => {
			const item = context.last;

			if ( attributeName == 'dashListChecked' && item.name == 'listItem' && item.getAttribute( 'listType' ) != 'dash' ) {
				return false;
			}
		} );

		// Register commands.
		editor.commands.add( 'dashList', new ListCommand( editor, 'dash' ) );
		editor.commands.add( 'dashListCheck', new DashListCheckCommand( editor ) );
		...
		// editor.keystrokes.set( 'Ctrl+space', () => editor.execute( 'dashListCheck' ) );

		// Remove `dashListChecked` attribute when a host element is no longer a to-do list item.
		...
		this.listenTo( model, 'applyOperation', ( evt, args ) => {
			...
			if ( operation.type == 'rename' && operation.oldName == 'listItem' ) {
				...
				if ( item.hasAttribute( 'dashListChecked' ) ) {
					listItemsToFix.add( item );
				}
			} else if ( operation.type == 'changeAttribute' && operation.key == 'listType' && operation.oldValue === 'dash' ) {
				for ( const item of operation.range.getItems() ) {
					if ( item.hasAttribute( 'dashListChecked' ) && item.getAttribute( 'listType' ) !== 'dash' ) {
						listItemsToFix.add( item );
					}
				}
			}
		...
		model.document.registerPostFixer( writer => {
			...
			for ( const listItem of listItemsToFix ) {
				writer.removeAttribute( 'dashListChecked', listItem );
				hasChanged = true;
			}
	 ...
	 * to toggle the `dashListChecked` attribute using the command, and restores the selection position.
	 *
...
function jumpOverCheckmarkOnSideArrowKeyPress( stopKeyEvent, model ) {
	...
	if ( parent.name === 'listItem' && parent.getAttribute( 'listType' ) == 'dash' && position.isAtStart ) {
